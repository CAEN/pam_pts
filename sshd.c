...
int login_permitted(char *user, struct passwd *pwd)
{
...
#ifdef USE_PAM_ACCT_MGMT
  {
    ...
    pam_error = pam_start("ssh",user,&PAM_conversation,&pamh);
    if (pam_error != PAM_SUCCESS) {
      log_msg("pam_start: error!\n",user);
      pam_end(pamh,0);
      packet_disconnect("PAM error during authorization.");
    }

    pam_error = pam_acct_mgmt(pamh,0);
    if (pam_error != PAM_SUCCESS) {
      pam_end(pamh,0);
      log_msg("LOGIN REFUSED for %.100s from %.200s",
              user, get_canonical_hostname());
      packet_disconnect("You are not authorized to use this service.");
    }
